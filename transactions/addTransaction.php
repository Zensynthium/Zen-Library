...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    } else if($availability == "1" && $input_isCheckOut == "1"){
      $isCheckOut_err = "You cannot Check Out a book that's already Checked Out.";
    } else if($availability == "0" && $input_isCheckOut == "0"){
      $isCheckOut_err = "You cannot Check In a book that's already Checked In.";
    } else {
    ...
    if(empty($input_memberID) && $isCheckOut == "0"){
    ...
    if(empty($bookID_err) && empty($transDate_err) && empty($memberID_err) && empty($isCheckOut_err) && empty($employeeID_err)){
        ...
        $sql = "INSERT INTO Transactions (BookID, MemberID, EmployeeID, TransDate, IsCheckOut) VALUES (?, ?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($conn, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "iiiss", $param_bookID, $param_memberID, $param_employeeID, $param_transDate, $param_isCheckOut);
      ...
      <div class="row">
          <div class="col-md-12">
            <div class="page-header">
              <h2 class="my-2">Add Transaction</h2>
            </div>
              ...
              <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        ...
                        <?php 
                          if ($bookArray){
                            for ($i = 0; $i < count($bookArray); $i++) {
                              echo '<option value="'. $bookArray[$i]['ID'] .'">'. $bookArray[$i]['ID'] . ' - '. $bookArray[$i]['Title'] . ' ('. (!$bookArray[$i]['Availability'] ? 'Checked In' : 'Checked Out')  .')</option>';
                            }
                          }
                        ?>
                      </select>
                        ...
                        <?php 
                          if ($memberArray){
                            for ($i = 0; $i < count($memberArray); $i++) {
                              echo '<option value="'. $memberArray[$i]['ID'] .'">'. $memberArray[$i]['ID'] . ' - '. $memberArray[$i]['Name'] .'</option>';
                            }
                          }
                        ?>
                      </select>
                        ...
                        <?php 
                          if ($employeeArray){
                            for ($i = 0; $i < count($employeeArray); $i++) {
                              echo '<option value="'. $employeeArray[$i]['ID'] .'">'. $employeeArray[$i]['ID'] . ' - '. $employeeArray[$i]['Name'] .'</option>';
                            }
                          }
                        ?>
                      </select>
                  ...
                  <div class="form-group <?php echo (!empty($isCheckOut_err)) ? 'has-error' : ''; ?>">
                      ...
                      <select name="isCheckOut" class="form-control">
                        <option value="" selected disabled hidden></option>
                        <option value="1">Checking Out Book</option>
                        <option value="0">Returning Book</option>
                      </select>
