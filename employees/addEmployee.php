...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($name_err) && empty($position_err) && empty($address_err) && empty($hireDate_err) && empty($phone_err)){
        // Prepare an insert statement
        $sql = "INSERT INTO Employees (name, position, address, phone, hireDate) VALUES (?, ?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($conn, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssss", $param_name, $param_position, $param_address, $param_phone, $param_hireDate);
...
<body>
    <!-- Navigation -->
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark text-center text-center">
      ...
      <div class="row">
          <div class="col-md-12">
            <div class="page-header">
              <h2 class="my-3">Add Employee</h2>
            </div>
              <p>Please fill this form and submit to add Employees into the database.</p>
